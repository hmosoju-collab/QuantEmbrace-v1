#!/bin/bash
# ============================================================
# strategy_engine.sh — UserData for strategy-engine EC2 service
...
IMAGE_URI="${ecr_base}/quantembrace-strategy_engine:latest-${environment}"
...
AWS_S3_MODEL_BUCKET=${s3_bucket_model_artifacts}
LOG_LEVEL=${log_level}
DYNAMODB_TABLE_PREFIX=${dynamodb_table_prefix}
KAFKA_BOOTSTRAP_SERVERS=$KAFKA_BOOTSTRAP_SERVERS
S3_BUCKET_MODEL_ARTIFACTS=${s3_bucket_model_artifacts}
# Required: candle signals are 7-12s old at risk_engine; 5s default rejects all.
RISK_MAX_SIGNAL_AGE_SECONDS=30
# NSE watchlist: comma-separated symbols for candle and tick strategy universe.
...
[Unit]
Description=QuantEmbrace ${service_name}
...
ExecStop=/usr/bin/docker stop --time=60 quantembrace-${service_name}
...
log "=== STRATEGY ENGINE STARTUP COMPLETE ==="
