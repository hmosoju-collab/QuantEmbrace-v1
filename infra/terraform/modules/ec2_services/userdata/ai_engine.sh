#!/bin/bash
# ============================================================
# ai_engine.sh — UserData for ai-engine EC2 service (Phase 6)
# Rendered by Terraform templatefile() — do not edit directly.
#
# Variables injected by Terraform:
#   environment, service_name, ecr_base, aws_region, log_level
#   dynamodb_table_prefix, s3_bucket_model_artifacts
#
# Boot sequence:
#   1. System deps + Docker
#   2. Kernel tuning (TCP + CPU governor)
#   3. CloudWatch Agent (logs + metrics)
#   4. ECR image pull
#   5. Kafka MSK bootstrap discovery
#   6. Environment file (includes model S3 bucket)
#   7. systemd unit (auto-restart on failure)
#
# Phase 6 design note:
#   ai_engine downloads model artifacts from S3 on startup via
#   ModelRegistry._load_from_s3().  On first boot before real models
#   are trained, it falls back to DummyClassifier stubs automatically.
#   No bootstrap failure for missing models — the service starts healthy.
# ============================================================
set -euo pipefail
...
# ── Kernel tuning ────────────────────────────────────────────────────────────
# TCP tuning matches other trading services for MSK Kafka throughput.
# CPU governor: set to performance on c6g (ARM Graviton) — avoids frequency
# scaling latency spikes during HMM inference bursts.
...
# Set CPU governor to performance (Graviton2 on c6g supports cpufreq)
...
IMAGE_URI="${ecr_base}/quantembrace-ai_engine:latest-${environment}"
...
# ── Runtime discovery: Kafka bootstrap ──────────────────────────────────────
APP_ENV="${environment}"
...
log "Kafka bootstrap resolved: $KAFKA_BOOTSTRAP_SERVERS"
...
S3_BUCKET_MODEL_ARTIFACTS=${s3_bucket_model_artifacts}
# Model hot-reload interval and enrichment tuning are read from DynamoDB
# strategy-config table at runtime — no static env vars needed here.
ENV
...
[Unit]
Description=QuantEmbrace ${service_name} (Phase 6 ML enrichment)
...
RestartSec=15s
TimeoutStartSec=180
# Longer stop timeout: model hot-reload loop and Kafka consumer close need time
TimeoutStopSec=90
...
ExecStop=/usr/bin/docker stop --time=90 quantembrace-${service_name}
...
log "=== AI ENGINE STARTUP COMPLETE ==="
